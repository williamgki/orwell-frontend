...
const SuggestedQuestions = ({ style, onQuestionClick }) => {
 const questions = {
   scholarly: [
     "What are the main themes in Orwell's work?",
     "How does Orwell view totalitarianism?",
...
export default function Home() {
 ...
 const handleSubmit = async (e) => {
   ...
   try {
     const response = await fetch('https://web-production-46b5.up.railway.app/api/chat', {
       method: 'POST',
       headers: { 
         'Content-Type': 'application/json',
         'Accept': 'application/json',
         'Origin': 'https://orwell-frontend.vercel.app'
       },
       body: JSON.stringify({ 
         question: input,
         style: style
 ...
 return (
   <main className="min-h-screen flex flex-col items-center p-4 bg-gradient-to-b from-gray-100 to-gray-200">
     <div className="w-full max-w-4xl bg-white/70 backdrop-blur-sm rounded-lg shadow-lg p-4">
       ...
       <h1 className="text-2xl font-bold text-center mb-4">Orwell Analysis</h1>
       ...
       <form onSubmit={handleSubmit} className="flex gap-2 mb-4">
         <input
           ...
           placeholder="Ask about Orwell's writing..."
